...
+ (void)checkOpenAlError:(NSString*)operation
{
    ...
    if (AL_NO_ERROR != error) {
        NSLog(@"Error %d when attempting to %@", error, operation);
    }
}

+ (void)playSoundNamed:(NSString *)name {
    //ALuint outputBuffer = (ALuint)[[soundBuffers objectForKey:name] intValue];
    printf("play outputBuffer id = %d\n", outputBuffer);
...
+ (void)initOpenAL {
    ...
    [self checkOpenAlError:@"open device"];
...
+ (void)loadSoundNamed:(NSString *)name withFileName:(NSString *)fileName andExtension:(NSString *)extension {
    ...
    NSString* filePath = [[[self class] frameworkBundle] pathForResource:fileName ofType:extension];
    ...
    printf("audio file size = %lld bytes\n", fileSizeInBytes);
    ...
    printf("outputBuffer id = %d\n", outputBuffer);
    
    // copy the data into the output buffer
    alBufferData(outputBuffer, AL_FORMAT_MONO16, audioData, bytesRead, 44100);
